...
int main(int argc, char **argv)
{
    ...
    {
	// simple 3 
	const int ins4[] = {20,11,25};
	const int del4[] = {11};
	unitDriver(ins4, sizeof ins4/sizeof(int), del4, sizeof del4 / sizeof(int));
    }
	        ...
	        const int del_keys[], const int num_deletes)
{
    ...
    dp = bst_access(del_tree, key);
    if(dp != NULL)
        printf(" dp: %p key: %d\n", dp, key);
    else
        printf("didn't find a key\n");

    printf("testing bst_size\n");
    printf("Number of items: %d Longest path: %d\n", 
		bst_size(del_tree), bst_int_path_len(del_tree));
...
/*
void ownDriver(const int ins_keys[], const int num_inserts,
	       const int del_keys[], const int num_deletes)
{
    int i, size, lenght;
    int *ip;
	...
	*ip = ins_keys[i];
    ...
    printf("testing bst_size\n");
    size = bst_size(del_tree);
    lenght = bst_int_path_len(dep_tree);
    printf("Number of items: %d\n Longest path: %d\n", size, lenght);

    printf("Removing %d itmes from tree\n", num_deletes);
    for (i=0;i < num_deletes; i++){
	printf(" -- Test(%d) about to remove ke %d\n", i, del_keys[i]);
	dp = bst_remove(del_tree, del_keys[i]);
	bst_debug_print_tree(del_tree);
	if(dp != NULL)
	    assert(*(int *)dp == del_keys[i]);
